...
# Since this is 2014, assume the last district
...
election.districts <- strsplit(candidates$election_districts, ",")
candidates$district <- sapply(election.districts, function(x) get_last(x))
...
candidates.14$age <- as.period(interval(as.Date(candidates.14$dob, format = "%m/%d/%Y"), "2014-11-14"), units = "year")$year

# For each district, get the youngest age and the average age
districts.14 <- candidates.14 %>% 
  group_by(district) %>%  
  summarise(youngest_age = max(age, na.rm = T),
            average_age = mean(age, na.rm = T))

warning("\nNo age data for district 4, filtering out.")
...
```{r}
logit <- glm(data = master, cbind(turnout, young_population - turnout) ~ youngest_age, family = "binomial")
summary(logit)
```


```{r}
logit2 <- glm(data = master, cbind(turnout, young_population - turnout) ~ average_age, family = "binomial")
summary(logit2)
```
...
- Get Census data. 
  ...
  - 2014 population from the 2015 ACS? [X]
